         ...
         <div class="w-full py-12 space-y-16">
            <div class="flex md:flex-row flex-col md:items-center items-start justify-between md:space-y-0 space-y-4">
               <div class="flex items-center justify-center space-x-4">
                  <Link href="/">
                  <img src="/images/back-arrow.png" class="w-14 h-14" /></Link>
   ...
   {{
      toMoney(
         (Number(s.total_amount || 0) +
            (Number(s.total_amount || 0) * Number(s.service_charge || 0) / 100)
         ) -
         (
            ...
            : Number(s.custom_discount || 0)
                  ...
                  <tfoot class="bg-gray-50 text-[12px] font-semibold">
                     <tr>
                        ...
                        <td class="p-3 text-center">{{ salesTotalQty.toLocaleString() }}
...
const salesGrossTotal = computed(() =>
  sales.value.reduce((a, s) => {
    ...
    // Service charge in LKR (if percentage)
    const serviceChargeLkr = (total * Number(s.service_charge || 0)) / 100;

    return a + (total + serviceChargeLkr - customDiscountLkr);
  }, 0)
);
