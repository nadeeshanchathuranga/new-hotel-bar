      ...
      <div class="flex flex-col justify-between items-center md:w-1/3 w-full bg-white border-4 border-black rounded-xl h-[450px]">
        <div class="chart-container w-full p-4">
          <div class="w-full flex justify-between items-center pb-8">
            ...
            <button @click="downloadEmployeeSalesPDF" class="w-full mt-6 px-4 py-2 text-md font-normal tracking-wider text-white bg-orange-600 rounded-lg custom-select hover:bg-orange-700 hover:shadow-lg">Download PDF</button>
          </div>
      ...
      <div class="flex flex-col justify-between items-center md:w-1/3 w-full bg-white border-4 border-black rounded-xl h-[450px]">
        <div class="chart-container w-full p-4">
          <div class="w-full flex justify-between items-center md:pt-12">
            ...
            <button @click="downloadProductQtyPDF" class="w-full mt-6 px-4 py-2 text-md font-normal tracking-wider text-white bg-orange-600 rounded-lg custom-select hover:bg-orange-700 hover:shadow-lg">Download PDF</button>
          </div>
      ...
      <div class="flex flex-col justify-between items-center md:w-1/3 w-full bg-white border-4 border-black rounded-xl h-[450px]">
        <div class="chart-container w-full p-4">
          <div class="w-full flex justify-between items-center md:pt-12">
            ...
            <button @click="downloadPaymentMethodPDF" class="w-full mt-6 px-4 py-2 text-md font-normal tracking-wider text-white bg-orange-600 rounded-lg custom-select hover:bg-orange-700 hover:shadow-lg">Download PDF</button>
          </div>
              ...
              <td class="p-3 num text-center">
                <ul>
                  <li class="font-semibold text-green-600">
                    <span v-if="s.is_service">{{ toMoney(s.total_amount || 0) }}</span>
                    <span v-else>{{ toMoney(saleProfit(s)) }}</span>
                  </li>
                </ul>
              </td>
            </tr>
...
/** ✅ jsPDF + AutoTable (explicit import; no side-effect patching) */
...
const props = defineProps({
  ...
  // ✅ add default to avoid runtime errors in export helpers
...
const totalSellingPrice = computed(() => products.value.reduce((sum, p) => sum + (p.stock_quantity || 0) * (parseFloat(p.selling_price) || 0), 0));
const totalRetailValue = computed(() => products.value.reduce((sum, p) => sum + (p.stock_quantity || 0) * (parseFloat(p.cost_price) || 0), 0));
...
const filterData = () => {
  ...
  // If you use Ziggy globally (window.route), replace route(...) with window.route(...)
  router.get(
    route("reports.index"),
...
// ===== Charts data =====
const sortDescending = (data) => Object.entries(data).sort((a, b) => b[1] - a[1]).reduce((acc, [k, v]) => ((acc[k] = v), acc), {});
  ...
  datasets: [
    {
      ...
      backgroundColor: [
        "#FF6384","#36A2EB","#FFCE56","#4BC0C0","#9966FF","#28a745","#ffc107","#17a2b8","#e83e8c","#fd7e14","#6610f2","#6f42c1","#dc3545","#adb5bd","#20c997","#ffc93c","#6a0572","#8ac926","#ff595e","#198754"
      ],
    },
  ],
}));
  ...
  datasets: [
    {
      ...
      backgroundColor: [
        "#FF6384","#36A2EB","#FFCE56","#4BC0C0","#9966FF","#28a745","#ffc107","#17a2b8","#e83e8c","#fd7e14","#6610f2","#6f42c1","#dc3545","#adb5bd","#20c997","#ffc93c","#6a0572","#8ac926","#198754"
      ],
    },
  ],
}));
...
const chartData4 = computed(() => ({
  ...
  datasets: [
    {
      data: Object.values(sortedEmployeeSales.value).map((e) => e["Total Sales Amount"]),
      backgroundColor: [
        "#6610f2","#36A2EB","#8ac926","#ff595e","#198754","#6f42c1","#dc3545","#adb5bd","#20c997","#28a745","#ffc107","#17a2b8","#e83e8c","#fd7e14","#FF6384","#FFCE56","#4BC0C0","#9966FF","#ffc93c"
      ],
    },
  ],
}));
...
// combined discount helpers (currently using only custom discount at sale-level)
...
// Net = (total + serviceCharge) - custom discount (LKR)
const netAmount = (s) => {
  const gross = Number(s.total_amount || 0);
  const svc = (gross * Number(s.service_charge || 0)) / 100;
  return gross + svc - customDiscountLkr(s);
};
...
const salesGrossTotal = computed(() =>
  (sales.value || []).reduce((a, s) => {
    ...
    const serviceChargeLkr = (total * Number(s.service_charge || 0)) / 100;
    const disc = customDiscountLkr(s);
    return a + (total + serviceChargeLkr - disc);
  }, 0)
);

const salesDiscountTotal = computed(() => (sales.value || []).reduce((a, s) => a + discountLkr(s), 0));
...
// Profit calc (sale-level; includes services)
const saleProfit = (s) => {
  const gross = Number(s.total_amount || 0);
  const svc = (gross * Number(s.service_charge || 0)) / 100;
  const discount = customDiscountLkr(s);
  const net = gross + svc - discount; // Final Selling Price in table
...
// ===== Chart PDFs (use explicit autoTable) =====
const downloadEmployeeSalesPDF = () => {
  const doc = new jsPDF();
  doc.text("Top Employee Sales", 14, 10);
  const rows = Object.entries(sortedEmployeeSales.value).map(([employee, entry]) => [employee, (entry["Total Sales Amount"] || 0).toLocaleString()]);
  autoTable(doc, { head: [["Employee", "Total Sales Amount"]], body: rows, startY: 20 });
...
const downloadProductQtyPDF = () => {
  ...
  autoTable(doc, { head: [["Product Name", "Quantity"]], body: rows, startY: 20 });
...
const downloadPaymentMethodPDF = () => {
  ...
  const rows = Object.entries(paymentMethodTotals.value).map(([m, t]) => [m, `LKR ${Number(t || 0).toLocaleString()}`]);
  autoTable(doc, { head: [["Payment Method", "Total Amount"]], body: rows, startY: 20 });
  doc.save("PaymentMethodTotals.pdf");
};

// ===== Sales table PDF (matches rendered table) =====
const downloadSalesTablePDF = () => {
  ...
  const _discountDisplay = (s) => {
    ...
    if (customVal > 0) {
      if (customType === "percent") {
        const lkr = (gross * customVal) / 100;
        parts.push(`${customVal}% (${_toMoney(lkr)} LKR)`);
      } else {
        parts.push(`${_toMoney(customVal)} LKR`);
      }
    }
    return parts.join("  |  ");
  };
...
const downloadStockTablePDF = () => {
  ...
  if (jq?.fn?.dataTable && jq.fn.dataTable.isDataTable("#stockQtyTbl")) {
    const dt = jq("#stockQtyTbl").DataTable();
  ...
  const totalsRow = [
    "",
    "Totals:",
    filteredTotals.qty.toLocaleString(),
...
.dataTables_wrapper .dataTables_paginate { display: flex; justify-content: center; align-items: center; margin-top: 20px; }
#salesTbl_filter, #stockQtyTbl_filter, #expenseTbl_filter { display: flex; justify-content: flex-end; align-items: center; margin-bottom: 16px; float: left; }
...
#salesTbl_filter input[type="search"], #stockQtyTbl_filter input[type="search"], #expenseTbl_filter input[type="search"] {
  font-weight: 400; padding: 9px 15px; font-size: 14px; color: #000000cc; border: 1px solid rgb(209 213 219); border-radius: 5px; background: #fff; outline: none; transition: all 0.5s ease;
}
#salesTbl_filter input[type="search"]:focus, #stockQtyTbl_filter input[type="search"]:focus, #expenseTbl_filter input[type="search"]:focus { border: 1px solid #4b5563; box-shadow: none; }
.dataTables_wrapper { margin-bottom: 10px; }
</style>

<style scoped>
.chart-container { display: flex; flex-direction: column; align-items: center; justify-content: center; width: 100%; height: calc(100% - 50px); position: relative; }
