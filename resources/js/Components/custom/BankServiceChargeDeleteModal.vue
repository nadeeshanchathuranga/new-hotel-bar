<template>
          ...
          >
            ...
            >
              ...
              <p class="text-[15px] text-center text-gray-700">
                Are you sure you want to delete this Bank Service Charge? This action cannot
                be undone.
              </p>
  ...
  // Props for the modal
  const { open, selectedBankCharge } = defineProps({
    open: {
      type: Boolean,
      required: true,
    },
  ...
  const deleteItem = () => {
  if (!selectedBankCharge?.id) return;

  form.delete(`/bank-service-charge/${selectedBankCharge.id}`, {
    onSuccess: () => {
